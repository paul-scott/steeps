...
model Hybrid_CSP_PV_Particle_HotH2_Burner_OptDispatch3
  ...
  parameter Real PV_fraction = 10.15591352 "Fraction of the hybrid system that is PV nameplate";
  parameter SI.Power P_hybrid_system = 3527000928 "Hybrid system nameplate [W] --> allow to be oversized";
  parameter Real t_storage_H2(unit = "h") = 272.7824446 "[ST] H2 Storage capacity";
  parameter Real t_storage(unit = "h") = 286.2140602 "[ST] Storage capacity";
  parameter Real PB_fraction = 0.3599182571;
  parameter Real upstreammultiplier_change = 24.03278332 "The multiplier controlled by the optimisation, and passed";
  ...
  parameter Real factor_multiplier = 3 "Multiplier to multiply heat ratio to LHV of cold H2";
  ...
  parameter SI.Temperature T_in_gas_DP_H2 = 537 + 273.15 "Design inlet temperature of the gas [K]";
  ...
  parameter SI.Mass m_max = if PB_fraction_final > 1e-3 then (m_dot_pcl_DP_H2_HX * 6 + m_flow_blk) * t_storage * 3600 else m_dot_pcl_DP_H2_HX * 6 * t_storage * 3600 "Max particles mass in tanks [kg]";
...
*/
  if time > 3.15359e7 then
    ...
    if on_CSP then
      ...
      eta_pb_gross := if PB_fraction_final > 1e-3 then E_pb_gross / E_pb_input else 0;
      eta_pb_net := if PB_fraction_final > 1e-3 then E_pb_net / E_pb_input else 0;
