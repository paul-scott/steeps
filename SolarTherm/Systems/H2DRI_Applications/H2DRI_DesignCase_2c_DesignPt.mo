...
model H2DRI_DesignCase_2c_DesignPt
  ...
  //changed
  parameter SI.SpecificEnthalpy h_OreD_hot_des = SolarTherm.Media.SolidParticles.IOE_Dehydroxylated_utilities.h_T(T_OreD_hot_des);
  ...
  parameter SI.SpecificEnthalpy h_H2_GGHX_max = Modelica.Media.IdealGases.SingleGases.H2.specificEnthalpy_pT(p_des, T_H2_mix_des) + (Q_flow_GGHX_des/eff_GGHX_des)/m_flow_H2_des;//Modelica.Media.IdealGases.SingleGases.H2.specificEnthalpy_pT(p_des, T_products_des) "Design specific enthalpy of H2 leaving the GGHX cold stream assuming it had been heated to the maximum possible temperature i.e. reactor product temperature (J/kg)";
  //GGHX Calculations where f_split_GGHX worth of offgas gets diverted to
  parameter SI.SpecificEnthalpy h_H2_pre1_des = Modelica.Media.IdealGases.SingleGases.H2.specificEnthalpy_pT(p_des, T_H2_mix_des) + Q_flow_GGHX_des/m_flow_H2_des;// = h_H2_mix_des + eff_GGHX_des * (h_H2_GGHX_max - h_H2_mix_des) "Design of H2 leaving the GGHX cold stream after considering HX effectiveness and assuming C_C is C_min (J/kg)";
  ...
  parameter SI.ThermalConductance C_C_GGHX = m_flow_H2_des * cp_Cold_H2_GGHX "Heat capacity rate of the cold H2 stream exiting the H2 mixer and entering the GGHX (W/K)";
  parameter SI.ThermalConductance C_H_GGHX = f_split_GGHX*m_flow_H2_excess * cp_Cold_H2_GGHX + f_split_GGHX*m_flow_H2O_des*cp_Hot_H2O_GGHX;
  //this is the one that C_GGHX_H must match.
  parameter Real f_split_GGHX = 1.0 - (m_flow_offgas_PGHX1/(m_flow_H2_excess+m_flow_H2O_des)); //C_C_GGHX / (m_flow_H2_excess * cp_Hot_H2_GGHX + m_flow_H2O_des * cp_Hot_H2O_GGHX) "Fraction of off-gas flow that is diverted to the GGHX in order to match C_C and C_H of that HX component";
  //results in C_r_GGHX
  parameter SI.HeatFlowRate Q_flow_GGHX_des = eff_GGHX_des * C_min_GGHX_des * (T_products_des - T_H2_mix_des) "Design heat transfer rate inside GGHX (J/s)";
  parameter SI.ThermalConductance C_min_GGHX_des = min(C_C_GGHX, C_H_GGHX);
  ...
  parameter SI.HeatFlowRate Q_flow_PGHX1_des = Q_flow_PGHX1_req;//eff_PGHX1_des * C_min_PGHX1 * (T_products_des - T_OreH_feedstock_des);
  ...
  parameter SI.Temperature T_OreH_med1_des = SolarTherm.Media.SolidParticles.IOE_Hydroxylated_utilities.T_h(h_OreH_med1_des) "Temperature of hydroxylated iron ore leaving PGHX1 and entering the med tank (K)";
  //done
  ...
  parameter SI.SpecificEnthalpy h_OreD_med_des = ((1.0 - f_mass_LOI_H)*m_flow_OreH_des*h_OreD_400C + m_flow_OreD_PGHX2*h_OreD_med2_des)/((1.0 - f_mass_LOI_H)*m_flow_OreH_des + m_flow_OreD_PGHX2) "Expected specific enthalpy of the med tank (J/kg)"; //Do this
  ...
  //parameter SI.SpecificEnthalpy h_OreD_med_des = 
  ...
  parameter SI.Temperature T_OreD_med_des = SolarTherm.Media.SolidParticles.IOE_Dehydroxylated_utilities.T_h(h_OreD_med_des) "Expected specific enthalpy of the med tank (K)";
  //done
  ...
  //PGHX1
  parameter SI.HeatFlowRate Q_dehydroxy_chem = n_flow_H2O_dehydroxy_des*(SolarTherm.Models.Chemistry.ChemTable.H2O.Hf0 + SolarTherm.Models.Chemistry.ChemTable.Fe2O3.Hf0 - SolarTherm.Models.Chemistry.ChemTable.Fe2O3H2O.Hf0);
  parameter SI.HeatFlowRate Q_dehydroxy_therm = m_flow_OreD_des*h_OreD_400C + m_flow_H2O_dehydroxy_des*(h_H2O_400C-h_H2O_ref) - m_flow_OreH_des*h_OreH_feedstock_des;
  
  parameter SI.HeatFlowRate Q_flow_PGHX1_req = Q_dehydroxy_chem + Q_dehydroxy_therm;
  
  parameter SI.SpecificHeatCapacity cp_H2H2O_PGHX1 = (m_flow_H2_excess/(m_flow_H2_excess+m_flow_H2O_des))*cp_Hot_H2_PGHX1 + (m_flow_H2O_des/(m_flow_H2_excess+m_flow_H2O_des))*cp_Hot_H2O_PGHX1;
  
  parameter SI.MassFlowRate m_flow_offgas_PGHX1 = Q_flow_PGHX1_req/(eff_PGHX1_des*cp_H2H2O_PGHX1*(T_products_des-T_amb_des));
  ...
  parameter Real FCI_GGHX = FOB_GGHX * 1.05 * 3.5 * 0.7012;
  //checked
  ...
  parameter Real FOB_heater = 0.0;
  //(CEPCI/816.0)*pri_heater*P_heater_max "FOB cost of the electrical heater (USD_year)";
...
end H2DRI_DesignCase_2c_DesignPt;