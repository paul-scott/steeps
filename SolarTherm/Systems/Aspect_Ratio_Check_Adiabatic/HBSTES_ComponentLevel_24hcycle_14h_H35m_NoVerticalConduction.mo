within SolarTherm.Systems.Aspect_Ratio_Check_Adiabatic;

model HBSTES_ComponentLevel_24hcycle_14h_H35m_NoVerticalConduction
  ...
  parameter SI.Temperature T_ext_max = 25.0 + 273.15  "External surface temperature of the HBS in the worst case scenario (K)";
  parameter SI.CoefficientOfHeatTransfer U_loss_top = 1e-9 "Heat loss coefficient at the top of the tank (W/m2K)";
  parameter SI.CoefficientOfHeatTransfer U_loss_bot = 1e-9 "Heat loss coefficient at the bottom of the tank (W/m2K)";
  ...
  //Tank Geometry
  parameter SI.Power Q_process_des = 26.6*(SolarTherm.Media.Air.Air_CoolProp_1bar_utilities.h_T(T_process_des)-SolarTherm.Media.Air.Air_CoolProp_1bar_utilities.h_T(T_min)) ;//E_max/t_discharge "Design required process heat-rate (W_th)";//E_max/t_discharge
  parameter SI.Power Q_heater_des = HM * Q_process_des "Design heater output heat-rate (W_th)";
  parameter SI.Energy E_max = Q_process_des*t_discharge*f_TES_oversize "Ideal storage capacity (J_thermal)"; //Note 3 tanks
...
end HBSTES_ComponentLevel_24hcycle_14h_H35m_NoVerticalConduction;