...
model HBSTES_ComponentLevel_24hcycle_14h_ar4p8
  ...
  parameter SI.Pressure p_drop_total_des = (8.0*f_des*((max(m_charge_des,m_discharge_des))^2.0)*(ar^4.0)/(d_p*rho_f_des*(eta^2.0)*(CN.pi*CN.pi)*((TES.Tank_A.H_tank)^3.0)));
  parameter SI.Velocity u_flow_des = max(m_charge_des,m_discharge_des)/(rho_f_des*eta*TES.Tank_A.A);
  parameter SI.Pressure p_drop_est = 32.0*mu_des*(TES.Tank_A.H_tank)*u_flow_des/(d_p*d_p);
...
end HBSTES_ComponentLevel_24hcycle_14h_ar4p8;