...
model WindPV_HBSTES_3TMixed_SystemModel "Generic system model with WindPV input, Annular TES component and a boiler"
  ...
  SolarTherm.Models.Storage.Thermocline.Parallel.Thermocline_HBS_LC_3P_MixedOutlet TES(redeclare package Medium = Medium, redeclare package Fluid_Package = Fluid, redeclare package Filler_Package_A = Filler, redeclare package Filler_Package_B = Filler, redeclare package Filler_Package_C = Filler, N_f_A = N_f, T_max = T_max, T_min = T_min, T_recv_set = T_heater_des, T_PB_set = T_process_des, Correlation = Correlation, E_max = E_max, d_p_A = d_p, s_p_A = s_p, U_loss_tank_A = U_loss_tank, frac_1 = 1.0 / 3.0, h_tol = h_tol) annotation(
